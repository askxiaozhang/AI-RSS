...
export default function ItemCard({
  ...
  return (
    <motion.article
      ...
      className={`group relative overflow-hidden rounded-xl border bg-white/90 backdrop-blur transition-all ${
        read
    ...
    >
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <a
              ...
              className="inline-flex items-center gap-1.5 text-base font-semibold text-slate-900 hover:text-brand-700"
            >
        ...
        {/* AI TL;DR */}
        {item.ai_tldr && (
          <div className="mt-3 flex gap-2 rounded-lg border border-brand-100 bg-brand-50/50 p-3">
            <Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-brand-500" />
            <p className="text-sm leading-relaxed text-slate-700">{item.ai_tldr}</p>
          </div>
        )}
        ...
        {(item.ai_summary || item.raw_content) && (
          <button
            ...
            className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-brand-700"
          >
            {expanded ? '收起' : '展开详情'}
