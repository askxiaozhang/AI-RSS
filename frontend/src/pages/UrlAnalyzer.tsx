...
// ---------------------------------------------------------------------------
// CSS injected into the iframe to drive both modes
// ---------------------------------------------------------------------------
const INSPECTOR_CSS = `
  /* select-mode only — activated via body._sel class */
  body._sel * { cursor: crosshair !important; user-select: none !important; }
  body._sel a, body._sel button { pointer-events: auto !important; }
  body._sel ._h {
...
function applyHighlights(doc: Document, selector: string): number {
  ...
  try {
    ...
    matches.forEach((el, i) => el.classList.add(i === 0 ? '_s' : '_m'))
...
export default function UrlAnalyzer() {
  ...
  const [urlInput, setUrlInput] = useState('')       // editable input
  const [loadedUrl, setLoadedUrl] = useState('')     // URL currently in iframe
  const [pageTitle, setPageTitle] = useState('')
  const [pageHtml, setPageHtml] = useState('')       // head+body from backend
  ...
  // ---------------------------------------------------------------------------
  // Load URL (used both for initial load and browse-mode navigation)
  // ---------------------------------------------------------------------------
  const loadUrl = useCallback(async (rawUrl: string) => {
    ...
    try {
      ...
      setPreviewError(err.response?.data?.detail || '无法加载该页面，请检查地址是否正确')
    } finally {
      setLoadingPreview(false)
    }
  }, [])

  // Keep ref in sync so iframe event handler can call it without stale closure
  useEffect(() => {
    navigateFnRef.current = loadUrl
  }, [loadUrl])
  ...
  const handleLoadPreview = async (e: React.FormEvent) => {
    ...
    pushMsg({
      type: 'info',
      text: '页面加载完成！\n\n• 浏览模式：可正常点击链接、浏览网站\n• 选择模式：点击任意内容区块，AI 自动识别同类重复元素\n\n右上角切换两种模式。',
    })
    setStep('inspect')
    try {
      setFeedTitle(new URL(urlInput.trim()).hostname.replace('www.', ''))
    } catch {}
  }

  // ---------------------------------------------------------------------------
  // Preview items for a selector
  // ---------------------------------------------------------------------------
  const loadPreviewForSelector = useCallback(
    async (sel: string, targetUrl: string) => {
      setLoadingItems(true)
      try {
        const { data } = await agentsApi.previewSelector(targetUrl, sel)
        setPreviewItems(data.items)
        setPreviewCount(data.count)
        if (data.count === 0) {
          pushMsg({ type: 'empty', text: `选择器 \`${sel}\` 没有匹配到内容，请换一个区块试试。` })
        } else {
          pushMsg({
            type: 'found',
            items: data.items.slice(0, 4),
            text: `识别到 **${data.count}** 个匹配条目（选择器：\`${sel}\`）。内容预览如下，这是你想要的吗？`,
          })
        }
      } catch {
        pushMsg({ type: 'error', text: '解析出错，请尝试选择其他区块或手动编辑选择器。' })
      } finally {
  ...
  // ---------------------------------------------------------------------------
  // Iframe load: inject CSS + unified event handler
  // ---------------------------------------------------------------------------
  const handleElementClick = useCallback(
    (el: Element, body: HTMLElement, doc: Document) => {
  ...
  // keep a stable ref so onIframeLoad closure stays fresh
  ...
  const onIframeLoad = useCallback(() => {
    ...
    // Inject inspector CSS once
    if (!doc.getElementById('_rss_inspector_css')) {
      const style = doc.createElement('style')
      style.id = '_rss_inspector_css'
    ...
    else body.classList.remove('_sel')
    ...
    const onClick = (e: MouseEvent) => {
      ...
      // Browse mode: intercept <a> clicks and load via fetch-preview
  ...
  }, [loadedUrl]) // loadedUrl for URL resolution in browse-mode clicks
  ...
  const switchMode = (next: Mode) => {
    ...
    if (!doc?.body) return
    if (next === 'select') {
      doc.body.classList.add('_sel')
    } else {
      doc.body.classList.remove('_sel')
      doc.querySelectorAll('._h').forEach((e) => e.classList.remove('_h'))
    }
  }

  // ---------------------------------------------------------------------------
  // Manual selector edit
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  const handleConfirm = () => {
    pushMsg({ type: 'confirm', text: `确认！将用 \`${selector}\` 提取 ${previewCount} 条内容。` })
    setStep('subscribe')
  }
  ...
  // srcdoc: full <html> document with head+body from backend
  const srcdoc = pageHtml
    ? `<!DOCTYPE html><html>${pageHtml}</html>`
    : ''

  // ============================================================
  // Render — Step 1: URL input
  // ============================================================
  if (step === 'url') {
    return (
      <div className="mx-auto max-w-2xl px-4 py-10">
        <div className="mb-8">
          ...
          <p className="mt-2 text-slate-500">
            输入网址后，可正常浏览网站，找到文章列表页面再切换到选择模式，点击任意文章卡片即可完成抓取配置。
          </p>
        </div>

        <form
          onSubmit={handleLoadPreview}
          className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur"
        >
          <div className="space-y-4">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">网站地址</label>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs text-slate-500 hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700"
                >
                  {ex.replace('https://', '')}
                </button>
              ))}
            ...
            >
              {loadingPreview ? (
                <><Loader2 className="h-4 w-4 animate-spin" />正在加载页面…</>
              ) : (
                <>
  ...
  // ============================================================
  // Render — Step 2: Interactive inspector
  // ============================================================
  if (step === 'inspect') {
    return (
          ...
          <div className="flex shrink-0 items-center gap-1.5 border-b border-slate-200 bg-slate-50 px-2 py-1.5">
            ...
            <form
              ...
              onSubmit={(e) => {
                e.preventDefault()
                loadUrl(urlInput)
                pushMsg({ type: 'nav', text: `正在加载：${urlInput}` })
              }}
            >
              <div className="flex min-w-0 flex-1 items-center gap-2 rounded-md border border-slate-200 bg-white px-2 py-1">
                ...
                <input
                  ...
                  spellCheck={false}
                />
                <button type="submit" className="text-slate-400 hover:text-brand-600">
            ...
            >
              {mode === 'browse' ? (
                <span className="flex items-center gap-1.5">
                  ...
                  浏览模式：点击链接正常跳转页面，找到文章列表后切换至「选择」模式
                </span>
              ) : (
                <span className="flex items-center gap-1.5">
                  <Crosshair className="h-3 w-3" />
                  选择模式：悬停高亮 · 点击选取文章卡片 · AI 自动识别同类条目
                </span>
              )}
            </motion.div>
          </AnimatePresence>
        ...
        <div className="flex w-[360px] shrink-0 flex-col bg-white">
          ...
          <div className="flex shrink-0 items-center gap-2 border-b border-slate-200 px-4 py-3">
            ...
            <div className="min-w-0">
              <div className="text-sm font-semibold text-slate-900">AI 抓取助手</div>
              <div className="truncate text-[11px] text-slate-400">{pageTitle}</div>
            </div>
          </div>
                ...
                >
                  <div className="flex items-start gap-2">
                    <div className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${
                      msg.type === 'nav' ? 'bg-slate-100' : 'bg-brand-100'
                    }`}>
                      {msg.type === 'nav'
                        ? <Navigation className="h-3.5 w-3.5 text-slate-500" />
                        : <Sparkles className="h-3.5 w-3.5 text-brand-600" />}
                    </div>
                    <div className={`min-w-0 flex-1 rounded-2xl rounded-tl-sm px-3 py-2.5 ${
                      msg.type === 'nav' ? 'bg-slate-50' : 'bg-slate-100'
                    }`}>
                      <p className="whitespace-pre-line text-[13px] leading-relaxed text-slate-700">
                        {msg.text.split(/(\*\*.*?\*\*|`.*?`)/g).map((part, i) => {
                          ...
                          if (part.startsWith('`') && part.endsWith('`'))
                            return (
                              <code key={i} className="rounded bg-slate-200 px-1 py-0.5 font-mono text-[11px] text-brand-700">
                                {part.slice(1, -1)}
                              </code>
                            )
                          return part
                            ...
                            <div key={i} className="rounded-lg border border-slate-200 bg-white px-2.5 py-2">
                              <div className="flex items-start gap-1.5">
                                ...
                                <div className="min-w-0">
                                  ...
                                  {item.link && (
                                    <p className="truncate text-[10px] text-brand-500">{item.link}</p>
                                  )}
                                  {item.description && (
          ...
          <div className="shrink-0 space-y-3 border-t border-slate-200 p-4">
            ...
            {selector && (
              <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                <div className="mb-1 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => { setEditingSelector((v) => !v); setSelectorInput(selector) }}
                    className="text-[11px] font-medium text-brand-600 hover:text-brand-700"
                  >
                    <Code2 className="mr-0.5 inline h-3 w-3" />
                    {editingSelector ? '取消' : '手动编辑'}
                  </button>
                </div>
                {editingSelector ? (
                  <div className="flex gap-1.5">
                    <input
                      ...
                      onKeyDown={(e) => e.key === 'Enter' && applyManualSelector()}
                      autoFocus
                    ...
                    />
                    <button
                      onClick={applyManualSelector}
                      className="rounded bg-brand-500 px-2 py-1 text-[11px] font-semibold text-white hover:bg-brand-600"
                    >
                      应用
                    </button>
                  </div>
            ...
            {/* Action buttons */}
            {selector && !loadingItems && previewCount > 0 ? (
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">
            将以{' '}
            <code className="rounded bg-slate-100 px-1 text-brand-700">{selector}</code>{' '}
            抓取 {previewCount} 条内容
          </p>
        </div>
      </div>
          ...
          {previewItems.length > 0 && (
            <div className="mb-5">
              <p className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-400">
                内容预览（前 {Math.min(previewItems.length, 3)} 条）
              </p>
                ...
                {previewItems.slice(0, 3).map((item, i) => (
                  <div key={i} className="flex items-start gap-2 rounded-lg bg-slate-50 px-3 py-2">
                    ...
                    <div className="min-w-0">
                      ...
                      {item.link && (
                        <p className="truncate text-[11px] text-brand-500">{item.link}</p>
                      )}
                    </div>
                  </div>
                ))}
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-emerald-700">
              「{feedTitle}」已添加，AI 将按设定间隔自动抓取更新。
            </p>
          </div>
          <button
            onClick={() => {
              setStep('url'); setUrlInput(''); setSelector(''); setPreviewItems([])
              setMessages([]); setSubscribed(false); setFeedTitle('')
            }}
