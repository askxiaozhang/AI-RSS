  ...
  if (step === 'url') {
    return (
      <div className="mx-auto max-w-2xl px-4 py-10">
        ...
        <form onSubmit={handleLoadPreview} className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur">
              ...
              {messages.map(msg => (
                <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.18 }}>
                  <div className="flex items-start gap-2">
                    ...
                    <div className={`min-w-0 flex-1 rounded-2xl rounded-tl-sm px-3 py-2.5 ${msg.type === 'nav' ? 'bg-slate-50' : 'bg-slate-100'}`}>
